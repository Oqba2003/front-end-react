...
function App() {
 ...
 <form onSubmit={handleAddProduct} className="space-y-4 mb-8 w-full max-w-md">
 <div>
...
{!productPrice && ( 
    <p className="text-red-500 text-sm mt-1">
      يا خاي السعر
        ...
        <button
          type="submit"
          className=" bg-green-400 text-white py-2 px-4 rounded-lg hover:bg-gray-300 transition-colors  "
        >
          Add Product
        </button>
      </form>
          ...
          <div key={product.id} className="flex items-center justify-between bg-white p-4 rounded-lg shadow">
            ...
            <div className="space-x-2">
              <button
                ...
                className="bg-blue-500 py-2 px-4 text-white rounded hover:bg-blue-300 transition-colors"
              >
                Increase Price
              </button>
              <button
                onClick={() => handleDelete(product.id)}
                className="bg-red-600 py-2 px-4 text-white rounded hover:bg-red-300 transition-colors"
              >
                Delete
              </button>
            </div>
